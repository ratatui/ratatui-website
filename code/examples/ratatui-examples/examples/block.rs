...
fn render_multiple_title_positions(paragraph: &Paragraph, frame: &mut Frame, area: Rect) {
    let block = Block::bordered()
        ...
        .title_bottom(Line::from("bottom left").alignment(Alignment::Left))
        .title_bottom(Line::from("bottom center").alignment(Alignment::Center))
        .title_bottom(Line::from("bottom right").alignment(Alignment::Right));
    frame.render_widget(paragraph.clone().block(block), area);
}
