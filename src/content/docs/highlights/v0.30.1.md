...
`CellWidth` also includes a terminal-compatibility fix for halfwidth katakana dakuten and
handakuten (`U+FF9E` / `U+FF9F`). While `unicode-width` reports those code points as zero-width,
terminals usually render them as occupying one cell, so strings like `ｶﾞ` and `ﾊﾟ` now report the
correct width.
