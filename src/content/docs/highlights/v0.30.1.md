...
usually render them as occupying one cell, so strings like `ｶﾞ` and `ﾊﾟ` now report the correct
width.
...
:::note[Ratzilla and WASM link errors]

[Ratzilla](https://github.com/ratatui/ratzilla) enables Ratatui's `layout-cache` feature. If you
build a Ratzilla app for `wasm32-unknown-unknown` and hit `critical-section` errors, add
`critical-section` with the `std` feature to the final app:

```toml
critical-section = { version = "1.2.0", features = ["std"] }
```

You can also disable Ratatui's `layout-cache` feature, but that may hurt layout performance.
