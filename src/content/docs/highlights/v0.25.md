...
We created an [awesome list](https://github.com/sindresorhus/awesome) for curating the awesome
applications/libraries built with `ratatui`!
...
- Simple: A template for bootstrapping a simply `ratatui` application quickly.
...
![ratatui-logo](https://raw.githubusercontent.com/ratatui-org/ratatui/images/examples/ratatui-logo.gif)
...
Watch the demo [here](https://github.com/ratatui-org/ratatui/blob/images/examples/colors_rgb.mov).
...
`Layout::segment_size(SegmentSize::EvenDistribution)`. The default behavior is
`SegmentSize::LastTakesRemainder`, which gives the last segment the remaining space.
...
- Add `#[must_use]` to Style-moving methods
