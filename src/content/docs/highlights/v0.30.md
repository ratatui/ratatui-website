...
We are excited to announce Ratatui 0.30.0!

See the [changelog](https://github.com/ratatui/ratatui/blob/main/CHANGELOG.md) for the full list of changes.

See the breaking changes for this release [here](https://github.com/ratatui/ratatui/blob/main/BREAKING-CHANGES.md).

## Modularization 🧩

Starting with Ratatui 0.30.0, the codebase was reorganized from a single monolithic crate into
...
Ratatui now supports `no_std` environments! 🎉

To build for embedded targets, disable the `default-features`:

```toml
ratatui = { version = "0.30.0", default-features = false }
```

TODO: expand this section cc. jagoda
...
- New constructors: `BarChart::new` , `BarChart::vertical`, `BarChart::horizontal`, `BarChart::grouped`, `Bar::new`, `Bar::with_label`, `BarGroup::new`, `BarGroup::with_label`
...
- Support customizable symbols via `LineGauge::filled_symbol` and `LineGauge::unfilled_symbol` methods
...
This is a breaking change and any code that uses conversion methods will need to be rewritten.
Since `Into::into` is not const, this function cannot be called in const context.
...
- [`examples/`](https://github.com/ratatui/ratatui/tree/main/examples): contains more complex application
and concept examples that are useful for getting inspiration for designing your own applications.

Also new examples such as [`mouse-drawing`](https://github.com/ratatui/ratatui/tree/main/examples/apps/mouse-drawing) and [`widget-ref-container`](https://github.com/ratatui/ratatui/tree/main/examples/apps/widget-ref-container) are added.
...
TODO: Make this section read better <https://github.com/ratatui/ratatui/commit/6dcd53bc6ba172d4b96d02809c6a46105e67d85a>
...
TODO: fix typo in https://github.com/ratatui/ratatui/pull/1929 and add visual tests & examples
...
We introduced `ratatui::run()` method which runs a closure with
...
Previously there was a blanket implementation of `Widget` for `WidgetRef`.
This has been reversed to instead be a blanket implementation of `WidgetRef` for all `&W` where `W: Widget`.
...
To call `Frame::render_widget_ref()` or `Frame::render_stateful_widget_ref()` you now neeed to import the `FrameExt` trait and enable the `unstable-widget-ref` feature.
...
- Last but not least, we have added a "Built with Ratatui" badge for downstream projects
...
_"Rats don't just survive; they discover; they create. ... I mean, just look at what they do with the terminal!" – Remy & Orhun_
