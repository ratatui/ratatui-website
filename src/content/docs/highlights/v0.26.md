...
There are brand new colors palettes ready to use!

### Material

The `ratatui::style::palette::material` module contains the Google 2014 Material Design palette.

See https://m2.material.io/design/color/the-color-system.html#tools-for-picking-colors for more
information.
...
The `ratatui::style::palette::tailwind` module contains the default Tailwind color palette. This is
useful for styling components with colors that match the Tailwind color palette.

See https://tailwindcss.com/docs/customizing-colors for more information on Tailwind.
...
We now have the following alignment convenience functions for `Line`, `Paragraph` and `Text`:
...
AsRef<Constraint>. This is useful when you want to specify a fixed size for a layout, but don't want
to explicitly create a Constraint::Length yourself.
...
The spacing can now be added between the items of a layout.
...
We now support a new way to space the elements in a `Layout`: Flex!

Loosely based on [flexbox](https://css-tricks.com/snippets/css/a-guide-to-flexbox/), this we added a
`Flex` enum with the following variants:
...
- `Min(v)` grows to allocate excess space in all `Flex` modes instead of shrinking (except in
  `Flex::Legacy` where it retains old behavior).
- `Fill(1)` grows to allocate excess space, growing equally with `Min(v)`.

We also removed the unstable feature `SegmentSize`.

This is a breaking change to the behavior of constraints. If you want the old behavior, you can use
`Flex::Legacy`:
...
If you want to perform hit tests, this new method is for you. (e.g. did the user clicked in an area)
...
Now you can split a `Rect` into multiple sub-`Rect`s:
...
It also means that any iterator whose item is convertible into `Row` can now be collected into a
`Table`.
...
We implemented `Widget` trait for references to widgets and changed their implementations to be
immutable.

This allows us to render widgets without consuming them by passing a ref to the widget when calling
`Frame::render_widget()`.
