...
let widgets: Vec<Box<dyn Widget>> = vec![Box::new(Line::raw("hello"), Span::raw("world"))];
...
With these changes, `Min(v)` grows to allocate excess space in all `Flex` modes instead of shrinking
(except in `Flex::Legacy` where it retains old behavior). `Fill(1)` grows to allocate excess space,
growing equally with `Min(v)`.
