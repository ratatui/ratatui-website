---
title: Calendar
---

Demonstrates the [`Calendar`](https://docs.rs/ratatui/0.30.2/ratatui/widgets/calendar/) widget.
Source
[calendar.rs](https://github.com/ratatui/ratatui/blob/ratatui-v0.30.2/ratatui-widgets/examples/calendar.rs).

```shell title=run example
git clone https://github.com/ratatui/ratatui.git --branch ratatui-v0.30.2 --depth 1
cd ratatui
cargo run -p ratatui-widgets --example calendar
```

![Calendar](https://github.com/ratatui/ratatui/blob/images/widget-examples/calendar.gif?raw=true)

```rust title=calendar.rs
{{ #include @code/examples/ratatui-examples/examples/calendar.rs }}
```
